...
public class Particulier extends Intervenants {
	...
	public static Particulier getParticulier(String noParticulier) {
		...
		try {
			...
			String sql;
			if (noParticulier != "") {
				sql = "Select * FROM Particuliers WHERE fldNoParticulier = "
						+ noParticulier;
			} else {
				sql = "Select * FROM Particuliers";
			}
	...
	public static ObservableList<Particulier> getAllParticulier() {
		ObservableList<Particulier> particuliers = FXCollections.observableArrayList();
		...
		try {
			...
			System.out.println("Creating statement");
			stmt = conn.createStatement();
			String sql = "Select * FROM Particuliers";
